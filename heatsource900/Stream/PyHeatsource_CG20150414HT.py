...
def GetSolarFlux(hour, JD, Altitude, Zenith, glorad, d_w, W_b, Elevation, TopoFactor,
    ...
    Air_Mass_Climate = (35 / sqrt(1224 * sin(radians(Altitude)) + 1)) * \
        exp(-0.0001184 * Elevation_Climate)
    Trans_Air = 0.0685 * cos((2 * pi / 365) * (JD + 10)) + 0.8
    F_Glo[1]=F_Glo[1]*Trans_Air**(Air_Mass_Climate*(exp(-0.000184*DeltaE)-1))
    ...
    #print (F_Direct[1])
        ...
        # 1 - Above Topography First calculation
    ...
    if F_Direct[0] == 0:
        ...
        Clearness_Index = F_Direct[8] / F_Direct[0]
    ...
    Trans_Stream = 0.5 - (0.194* log10(Water_Path * 100))
    if Trans_Stream > 1:
        Trans_Stream = 1
    if Trans_Stream < 0: 
        Trans_Stream =0 
    ...
    Trans_Stream = 0.5 - (0.194* log10(100 * d_w))
    if Trans_Stream > 1:
        Trans_Stream = 1
    if Trans_Stream < 0 : 
        Trans_Stream=0
...
def CalcHeatFluxes(ContData, C_args, d_w, area, P_w, W_w, U, Q_tribs, T_tribs, T_prev,
    ...
    glorad, wind, humidity, T_air, cloud = ContData
