...
def CalcSolarPosition(lat, lon, hour, min, sec, offset, JDC, radial_count):
    print ("running CalcSolarPosition")  
    toRadians = pi/180.0
    toDegrees = 180.0/pi
...
def GetStreamGeometry(Q_est, W_b, z, n, S, D_est, dx, dt):
    print ("running GetStreamGeometry")  
    Converge = 10
    dy = 0.01
    count = 0
    power = 2/3
...
def GetSolarFlux(hour, JD, Altitude, Zenith, glorad, d_w, W_b, Elevation, TopoFactor,
    ...
    with open('/usr/local/lib/python2.7/dist-packages/heatsource900/Stream/pot_rows.txt', 'r') as f1:
         data1 = f1.readlines()
    f1.closed
    ...
    with open('/usr/local/lib/python2.7/dist-packages/heatsource900/Stream/ratio_glopot.txt', 'r') as f1:
         data1 = f1.readlines()
    f1.closed
    ...
    with open ('/usr/local/lib/python2.7/dist-packages/heatsource900/Stream/ratio_dirglo.txt', 'r') as f2:
             data2 = f2.readlines()
    f2.closed
