...
class Output(object):
    ...
    def __init__(self, reach, start_time, run_type):
        ...
        if run_type < 2:
            ...
            desc["Heat_SR2.2"] = "Above Landcover: Solar Radiation Flux (w/sq m) First Calculation method"
            desc["Cloud"] = "Cloudiness"
    ...
    def __call__(self, time, hour):
        ...
        if self.run_type < 2:
            ...
            data["Heat_SR2.2"][timestamp] = [x.F_Solar[8] for x in nodes]
            data["Cloud"][timestamp] = [x.cloud for x in nodes]
